...
func readPids(path string, subsystem cgroups.Name) ([]cgroups.Process, error) {
	...
	for s.Scan() {
		if t := s.Text(); t != "" {
			...
			out = append(out, cgroups.Process{
				Pid:       pid,
				Subsystem: subsystem,
				Path:      path,
			})
...
func parseKV(raw string) (string, uint64, error) {
	...
	default:
		return "", 0, cgroups.ErrInvalidFormat
	}
}
...
func getCgroupDestination(subsystem string) (string, error) {
	...
	return "", cgroups.ErrNoCgroupMountDestination
}
...
func initializeSubsystem(s cgroups.Subsystem, path cgroups.Path, resources *specs.LinuxResources) error {
